...
/*   Updated: 2022/01/13 15:48:03 by nkolle           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_check_validmap_walls(t_game *game)
{
	...
	{
		if (x < game->fields.f_col - 1)
			x++;
		if (y < game->fields.f_row - 1 && x == game->fields.f_col - 1)
			y++;
		if (x == game->fields.f_col - 1 && y == game->fields.f_row - 1)
			break ;
	}	
	while (game->map.map[y][x] == '1')
	{
		if (y <= game->fields.f_row && x > 0)
			x--;
		if (x == 0 && y > 0)
			y--; 
		if (x == 0 && y == 0)
