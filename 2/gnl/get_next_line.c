...
/*   Updated: 2021/11/11 17:01:30 by nkolle           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_buf_update(char *buf)
{
	...
	temp = ft_substr(buf, ft_strlen2(buf) + 1, ft_strlen(buf));
	if (buf)
		free (buf);
	return (temp);
}
...
char	*ft_safe(char **buf, char **new_line, int by_co)
{
	...
	if (*new_line)
		free(*new_line);
	*new_line = ft_substr(*buf, 0, ft_strlen(*buf));
	*buf[0] = '\0';
	return (*new_line);
}
...
char	*get_next_line(int fd)
{
	...
	while (1)
	{
		...
		{
			new_line = ft_substr(buf, 0, ft_strlen2(buf) + 1);
